...
To fully support a new resource in the terraform-google-conversion tool, you must implement support for it in both tfplan2cai and cai2hcl.

`tfplan2cai` Support: The resource must have an entry in the [`pkg/tfplan2cai/converters/resource_converters.go` file](https://github.com/GoogleCloudPlatform/terraform-google-conversion/blob/main/pkg/tfplan2cai/converters/resource_converters.go). You can confirm this by searching that file for the Terraform resource name, such as `google_alloydb_backup`.

`cai2hcl` Support: The resource must have an entry in the [`pkg/cai2hcl/converters/resource_converters.go` file](https://github.com/GoogleCloudPlatform/terraform-google-conversion/blob/main/pkg/cai2hcl/converters/resource_converters.go). You can verify this by searching for the corresponding Cloud Asset type, such as `alloydb.googleapis.com/Backup`.
...
If an autogenerated converters are not possible, you can instead place three handwritten files in the specific Product folder in the [`mmv1/third_party/tgc_next/pkg/services` folder](https://github.com/GoogleCloudPlatform/magic-modules/tree/main/mmv1/third_party/tgc_next/pkg/services).
...
##### Address integrations tests failures
...
* Structural and Definition Overrides
...
* Field-Specific Conversion Logic
	...
	These rules are used when custom code is required because the field values or structure change during the forward or reverse conversion process.

	* Ignore Field Missing in CAI

		If a field is present in the Terraform resource but simply not supported or exported by Cloud Asset Inventory (CAI), add `is_missing_in_cai: true` to that specific field's definition in the Resource.yaml file.
	...
	* Custom Decoder (CAI → HCL Mismatch)

		This is used when the value of a field in a CAI asset is different from the value required in the final API object (reverse conversion).
	...
	* Custom Expander (CREATE Mismatch)

		This is used when the value of a field in a CAI asset is different from the value needed in the API object when the resource is first created.
	...
	* TGC Specific Expander / Flattener
		...
		For custom forward conversion logic (HCL to CAI/API), add `custom_tgc_expand` to the field.

		For custom reverse conversion logic (CAI/API to HCL), add `custom_tgc_flatten` to the field.
...
If the `tgc-test` is failing, make sure you can run the unit tests successfully locally. If any of the `tgc-test-integration-*` is failing, make sure you can run the integration tests successfully locally.
