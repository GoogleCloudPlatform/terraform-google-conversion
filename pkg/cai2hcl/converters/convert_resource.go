...
func ConvertResource(assets []caiasset.Asset, options *models.ResourceConverterOptions) ([]byte, error) {
	if len(assets) == 0 {
		return nil, nil
	}
	...
	if len(assets) > 1 {
		...
		return nil, fmt.Errorf("multiple assets are not supported yet.")
	}
	...
	if len(converters) == 1 {
		...
		case "compute.googleapis.com/InstantSnapshot":
			if true && strings.Contains(asset.Name, "zones") {
				converter = ConverterMap[asset.Type]["ComputeInstantSnapshot"]
			} else if true && strings.Contains(asset.Name, "regions") {
				converter = ConverterMap[asset.Type]["ComputeRegionInstantSnapshot"]
			}
	...
	newBlocks, err := converter.Convert(assets, options)
	if err != nil {
		return nil, err
	}
	if len(newBlocks) > 0 {
