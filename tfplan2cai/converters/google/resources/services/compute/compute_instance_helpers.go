...
func flattenIpv6AliasRange(d *schema.ResourceData, ranges []interface{}, i int) []map[string]interface{} {
	prefix := fmt.Sprintf("network_interface.%d", i)
	...
	for _, raw := range ranges {
		...
		ipCidrRange, ok := ipRange["ipCidrRange"].(string)
		if !ok && ipRange["ipCidrRange"] != nil {
			log.Printf("[WARN] flattenIpv6AliasRange: unexpected type for ipCidrRange: %T", ipRange["ipCidrRange"])
		}
		subnetworkRangeName, ok := ipRange["subnetworkRangeName"].(string)
		if !ok && ipRange["subnetworkRangeName"] != nil {
			log.Printf("[WARN] flattenIpv6AliasRange: unexpected type for subnetworkRangeName: %T", ipRange["subnetworkRangeName"])
		}
		apiData = append(apiData, map[string]interface{}{
			"ip_cidr_range":         ipCidrRange,
			"subnetwork_range_name": subnetworkRangeName,
...
func flattenScheduling(resp map[string]interface{}) []map[string]interface{} {
	...
	if gs, ok := resp["gracefulShutdown"].(map[string]interface{}); ok {
		schedulingMap["graceful_shutdown"] = flattenGracefulShutdown(gs)
	}
...
func flattenGracefulShutdown(v map[string]interface{}) []interface{} {
	...
	if md, ok := v["maxDuration"].(map[string]interface{}); ok {
...
func flattenAccessConfigs(accessConfigs []interface{}) ([]map[string]interface{}, string) {
	...
	for _, raw := range accessConfigs {
		...
		} else if !ok && ac["securityPolicy"] != nil {
			log.Printf("[WARN] flattenAccessConfigs: unexpected type for securityPolicy: %T", ac["securityPolicy"])
		}
...
func flattenIpv6AccessConfigs(ipv6AccessConfigs []interface{}) []map[string]interface{} {
	...
	for _, raw := range ipv6AccessConfigs {
		...
		if sp, ok := ac["securityPolicy"].(string); ok && sp != "" {
			...
			log.Printf("[WARN] flattenIpv6AccessConfigs: unexpected type for securityPolicy: %T", ac["securityPolicy"])
		}
		flattened = append(flattened, entry)
	}
	return flattened
...
func flattenNetworkInterfaces(d *schema.ResourceData, config *transport_tpg.Config, networkInterfaces []interface{}) ([]map[string]interface{}, string, string, string, error) {
	...
	for i, raw := range networkInterfaces {
		...
		} else if !ok && iface["macAddress"] != nil {
			log.Printf("[WARN] flattenNetworkInterfaces: unexpected type for macAddress at index %d: %T", i, iface["macAddress"])
		}
		if internalIP == "" {
		...
		if sp := firstAccessConfigSecurityPolicy(accessConfigs); sp != "" {
			flattened[i]["security_policy"] = sp
		} else if sp := firstAccessConfigSecurityPolicy(ipv6AccessConfigs); sp != "" {
			flattened[i]["security_policy"] = sp
		}
	}
...
func firstAccessConfigSecurityPolicy(accessConfigs []interface{}) string {
	if len(accessConfigs) == 0 {
		return ""
	}
	ac, ok := accessConfigs[0].(map[string]interface{})
	if !ok {
		return ""
	}
	sp, ok := ac["securityPolicy"].(string)
	if !ok && ac["securityPolicy"] != nil {
		log.Printf("[WARN] firstAccessConfigSecurityPolicy: unexpected type for securityPolicy: %T", ac["securityPolicy"])
	}
	return sp
}
