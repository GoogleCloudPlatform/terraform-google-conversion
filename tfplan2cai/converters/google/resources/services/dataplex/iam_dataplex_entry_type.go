...
// NewDataplexEntryTypeIamMemberResource returns the google_dataplex_entry_type_iam_member
// managed resource. It is shared by the managed resource registration and the
// list resource, so both stay in sync.
func NewDataplexEntryTypeIamMemberResource() *schema.Resource {
	return tpgiamresource.ResourceIamMember(
		DataplexEntryTypeIamSchema,
		DataplexEntryTypeIamUpdaterProducer,
		DataplexEntryTypeIdParseFunc,
		tpgiamresource.IamWithParentResourceIdentity(DataplexEntryTypeIamParentParentResourceIdentityParser),
	)
}
...
func DataplexEntryTypeIamUpdaterProducer(d tpgresource.TerraformResourceData, config *transport_tpg.Config) (tpgiamresource.ResourceIamUpdater, error) {
	...
	if v, ok := d.GetOk("entry_type_id"); ok {
		values["entry_type_id"] = v.(string)
	}

	// We may have gotten either a long or short name, so attempt to parse long name if possible
	m, err := tpgresource.GetImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/entryTypes/(?P<entry_type_id>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<entry_type_id>[^/]+)", "(?P<location>[^/]+)/(?P<entry_type_id>[^/]+)", "(?P<entry_type_id>[^/]+)"}, d, config, d.Get("entry_type_id").(string))
	if err != nil {
		return nil, err
	}
	...
	u := &DataplexEntryTypeIamUpdater{
		project:     values["project"],
		location:    values["location"],
		entryTypeId: values["entry_type_id"],
		d:           d,
		Config:      config,
	}
	...
	if err := d.Set("entry_type_id", u.GetResourceId()); err != nil {
		return nil, fmt.Errorf("Error setting entry_type_id: %s", err)
	}
...
func DataplexEntryTypeIdParseFunc(d *schema.ResourceData, config *transport_tpg.Config) error {
	...
	m, err := tpgresource.GetImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/entryTypes/(?P<entry_type_id>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<entry_type_id>[^/]+)", "(?P<location>[^/]+)/(?P<entry_type_id>[^/]+)", "(?P<entry_type_id>[^/]+)"}, d, config, d.Id())
	if err != nil {
		return err
	}
	...
	if err := d.Set("entry_type_id", u.GetResourceId()); err != nil {
		return fmt.Errorf("Error setting entry_type_id: %s", err)
	}
...
func (u *DataplexEntryTypeIamUpdater) GetResourceIamPolicy() (*cloudresourcemanager.Policy, error) {
	url, err := u.qualifyEntryTypeUrl("getIamPolicy")
	if err != nil {
		return nil, err
	}
...
func (u *DataplexEntryTypeIamUpdater) SetResourceIamPolicy(policy *cloudresourcemanager.Policy) error {
	...
	url, err := u.qualifyEntryTypeUrl("setIamPolicy")
	if err != nil {
		return err
	}
	project, err := tpgresource.GetProject(u.d, u.Config)
...
func (u *DataplexEntryTypeIamUpdater) qualifyEntryTypeUrl(methodIdentifier string) (string, error) {
	urlTemplate := fmt.Sprintf("{{DataplexBasePath}}%s:%s", fmt.Sprintf("projects/%s/locations/%s/entryTypes/%s", u.project, u.location, u.entryTypeId), methodIdentifier)
...
func (u *DataplexEntryTypeIamUpdater) GetMutexKey() string {
	return fmt.Sprintf("iam-dataplex-entrytype-%s", u.GetResourceId())
}

func (u *DataplexEntryTypeIamUpdater) DescribeResource() string {
	return fmt.Sprintf("dataplex entrytype %q", u.GetResourceId())
}
