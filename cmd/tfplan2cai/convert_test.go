...
func testAssets(path, project, zone, region string, ancestry map[string]string, offline, convertUnchanged bool, errorLogger *zap.Logger, userAgent string) []caiasset.Asset {
...
func mockConvertAssets(ctx context.Context, path, project, zone, region string, ancestry map[string]string, offline, convertUnchanged bool, errorLogger *zap.Logger, userAgent string) ([]caiasset.Asset, error) {
...
func TestConvertRun(t *testing.T) {
	...
	expectedAssetJSON, _ := json.Marshal(testAssets(path, "", "", "", map[string]string{}, false, false, errorLogger, "tfplan2cai"))
...
func TestConvertRun_passesCorrectArguments(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			var expectedAssets []interface{}
			expectedAssetJSON, _ := json.Marshal(testAssets(path, c.wantProject, c.wantZone, c.wantRegion, wantAncestryCache, false, false, errorLogger, "tfplan2cai"))
			json.Unmarshal(expectedAssetJSON, &expectedAssets)
			a.Equal(expectedAssets, output["resource_body"])
		})
	}
