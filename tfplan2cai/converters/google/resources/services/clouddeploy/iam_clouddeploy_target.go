...
func ClouddeployTargetIamUpdaterProducer(d tpgresource.TerraformResourceData, config *transport_tpg.Config) (tpgiamresource.ResourceIamUpdater, error) {
	...
	m, err := tpgresource.GetImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/targets/(?P<name>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<name>[^/]+)", "(?P<location>[^/]+)/(?P<name>[^/]+)", "(?P<name>[^/]+)"}, d, config, d.Get("name").(string))
	if err != nil {
		return nil, err
	}
...
func ClouddeployTargetIdParseFunc(d *schema.ResourceData, config *transport_tpg.Config) error {
	...
	m, err := tpgresource.GetImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/targets/(?P<name>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<name>[^/]+)", "(?P<location>[^/]+)/(?P<name>[^/]+)", "(?P<name>[^/]+)"}, d, config, d.Id())
	if err != nil {
		return err
	}
...
func (u *ClouddeployTargetIamUpdater) GetResourceIamPolicy() (*cloudresourcemanager.Policy, error) {
	url, err := u.qualifyTargetUrl("getIamPolicy")
	if err != nil {
		return nil, err
	}
...
func (u *ClouddeployTargetIamUpdater) SetResourceIamPolicy(policy *cloudresourcemanager.Policy) error {
	...
	url, err := u.qualifyTargetUrl("setIamPolicy")
	if err != nil {
		return err
	}
	project, err := tpgresource.GetProject(u.d, u.Config)
...
func (u *ClouddeployTargetIamUpdater) qualifyTargetUrl(methodIdentifier string) (string, error) {
	urlTemplate := fmt.Sprintf("{{ClouddeployBasePath}}%s:%s", fmt.Sprintf("projects/%s/locations/%s/targets/%s", u.project, u.location, u.name), methodIdentifier)
...
func (u *ClouddeployTargetIamUpdater) GetResourceId() string {
	return fmt.Sprintf("projects/%s/locations/%s/targets/%s", u.project, u.location, u.name)
}

func ClouddeployTargetIamParentParentResourceIdentityParser(d *schema.ResourceData, identity *schema.IdentityData, transportConfig *transport_tpg.Config) (string, error) {
	return tpgiamresource.ParseIamResourceIdentity(d, identity, transportConfig, tpgiamresource.IamResourceIdentityConfig{
		...
		UriFormat: "projects/%s/locations/%s/targets/%s",
	})
}

func (u *ClouddeployTargetIamUpdater) GetMutexKey() string {
	return fmt.Sprintf("iam-clouddeploy-target-%s", u.GetResourceId())
}

func (u *ClouddeployTargetIamUpdater) DescribeResource() string {
	return fmt.Sprintf("clouddeploy target %q", u.GetResourceId())
}
