...
import (
	...
	"github.com/GoogleCloudPlatform/terraform-google-conversion/v6/cmd/model"
...
func newRootCmd() (*cobra.Command, *model.RootOptions, error) {
	o := &model.RootOptions{
	...
	cmd := &cobra.Command{
		...
		PersistentPreRunE: func(c *cobra.Command, args []string) error {
			...
			errorLogger := model.NewErrorLogger(o.Verbosity, o.UseStructuredLogging, zapcore.Lock(os.Stderr))
			defer errorLogger.Sync()
			zap.RedirectStdLog(errorLogger)
			o.ErrorLogger = errorLogger

			outputLogger := model.NewOutputLogger(zapcore.Lock(os.Stdout))
