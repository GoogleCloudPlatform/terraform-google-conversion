...
// NewDataplexTaskIamMemberResource returns the google_dataplex_task_iam_member
// managed resource. It is shared by the managed resource registration and the
// list resource, so both stay in sync.
func NewDataplexTaskIamMemberResource() *schema.Resource {
	return tpgiamresource.ResourceIamMember(
		DataplexTaskIamSchema,
		DataplexTaskIamUpdaterProducer,
		DataplexTaskIdParseFunc,
		tpgiamresource.IamWithParentResourceIdentity(DataplexTaskIamParentParentResourceIdentityParser),
	)
}
...
func DataplexTaskIamUpdaterProducer(d tpgresource.TerraformResourceData, config *transport_tpg.Config) (tpgiamresource.ResourceIamUpdater, error) {
	...
	if v, ok := d.GetOk("task_id"); ok {
		values["task_id"] = v.(string)
	}

	// We may have gotten either a long or short name, so attempt to parse long name if possible
	m, err := tpgresource.GetImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/lakes/(?P<lake>[^/]+)/tasks/(?P<task_id>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<lake>[^/]+)/(?P<task_id>[^/]+)", "(?P<location>[^/]+)/(?P<lake>[^/]+)/(?P<task_id>[^/]+)", "(?P<task_id>[^/]+)"}, d, config, d.Get("task_id").(string))
	if err != nil {
		return nil, err
	}
	...
	if err := d.Set("task_id", u.GetResourceId()); err != nil {
		return nil, fmt.Errorf("Error setting task_id: %s", err)
	}
...
func DataplexTaskIdParseFunc(d *schema.ResourceData, config *transport_tpg.Config) error {
	...
	m, err := tpgresource.GetImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/lakes/(?P<lake>[^/]+)/tasks/(?P<task_id>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<lake>[^/]+)/(?P<task_id>[^/]+)", "(?P<location>[^/]+)/(?P<lake>[^/]+)/(?P<task_id>[^/]+)", "(?P<task_id>[^/]+)"}, d, config, d.Id())
	if err != nil {
		return err
	}
...
func (u *DataplexTaskIamUpdater) GetResourceIamPolicy() (*cloudresourcemanager.Policy, error) {
	url, err := u.qualifyTaskUrl("getIamPolicy")
	if err != nil {
		return nil, err
	}
...
func (u *DataplexTaskIamUpdater) SetResourceIamPolicy(policy *cloudresourcemanager.Policy) error {
	...
	url, err := u.qualifyTaskUrl("setIamPolicy")
	if err != nil {
		return err
	}
	project, err := tpgresource.GetProject(u.d, u.Config)
...
func (u *DataplexTaskIamUpdater) qualifyTaskUrl(methodIdentifier string) (string, error) {
	urlTemplate := fmt.Sprintf("{{DataplexBasePath}}%s:%s", fmt.Sprintf("projects/%s/locations/%s/lakes/%s/tasks/%s", u.project, u.location, u.lake, u.taskId), methodIdentifier)
...
func (u *DataplexTaskIamUpdater) DescribeResource() string {
	return fmt.Sprintf("dataplex task %q", u.GetResourceId())
}
