...
func GetTagsTagValueCaiObject(d TerraformResourceData, config *Config) ([]Asset, error) {
	name, err := assetName(d, config, "//tags.googleapis.com/tagValues/{{name}}")
	if err != nil {
		return []Asset{}, err
	}
	if obj, err := GetTagsTagValueApiObject(d, config); err == nil {
		return []Asset{{
			Name: name,
...
func GetTagsTagValueApiObject(d TerraformResourceData, config *Config) (map[string]interface{}, error) {
